...
const IMAGE_FILE_NAMES_KEY = 'campaign-register-image-names';
...
export const saveImageFileNames = (fileNames: StoredImageFileNames): void => {
  ...
  try {
    ...
    warn('campaignImageStorage', 'Failed to save image file names to sessionStorage:', error);
  }
};
...
export const getStoredImageFileNames = (): StoredImageFileNames => {
  ...
  try {
    ...
    warn('campaignImageStorage', 'Failed to restore image file names from sessionStorage:', error);
  }
  return {};
};
