...
export const useModelImageManagement = ({ onImageRestored }: UseModelImageManagementOptions = {}) => {
  ...
  useEffect(() => {
    ...
    debug('useModelImageManagement', '🖼️ 이미지 복원 체크 시작', {
    ...
    if (currentStoredImages.mainThumbnail && currentStoredImages.mainThumbnail !== mainThumbnailUrl) {
      debug('useModelImageManagement', '🖼️ mainThumbnail 이미지 복원', {
    ...
    if (currentStoredImages.gallery.length > 0 && JSON.stringify(currentStoredImages.gallery) !== JSON.stringify(galleryImageUrls)) {
      debug('useModelImageManagement', '🖼️ gallery 이미지 복원', {
    ...
    if (hasRestoredImage) {
      debug('useModelImageManagement', '🖼️ 이미지 복원 완료, onImageRestored 호출');
      onImageRestored?.();
      // 폼 데이터 복원 후 이미지 URL 삭제 (약간의 지연을 두어 복원이 완료되도록)
      setTimeout(() => {
        clearImageUrls();
        debug('useModelImageManagement', '🖼️ 이미지 복원 완료, 이미지 URL 삭제');
      }, 100);
    } else {
      debug('useModelImageManagement', '⏭️ 이미지 복원 없음');
    }
  ...
  const handleMainThumbnailSelect = useCallback(
    (file: File) => {
      ...
      debug('useModelImageManagement', '🖼️ handleMainThumbnailSelect 호출', {
        fileName: file.name,
        fileSize: file.size,
      });
      ...
      debug('useModelImageManagement', '✅ mainThumbnail 이미지 저장 완료', {
        hasFile: !!file,
        blobUrl,
        fileName: file.name,
        fileSize: file.size,
  ...
  const handleGalleryImageSelect = useCallback(
    (file: File) => {
      ...
      debug('useModelImageManagement', '🖼️ handleGalleryImageSelect 호출', {
        fileName: file.name,
        fileSize: file.size,
      ...
      if (galleryImageUrls.length >= 5) {
        debug('useModelImageManagement', '⚠️ 갤러리 이미지 최대 개수 초과');
        return false;
      }
      ...
      debug('useModelImageManagement', '✅ gallery 이미지 저장 완료', {
        hasFile: !!file,
        blobUrl,
        fileName: file.name,
        fileSize: file.size,
  ...
  const handleGalleryImageReplace = useCallback(
    (index: number, file: File) => {
      ...
      debug('useModelImageManagement', '🖼️ handleGalleryImageReplace 호출', {
        index,
        fileName: file.name,
        fileSize: file.size,
      });
      ...
      debug('useModelImageManagement', '✅ gallery 이미지 교체 완료', {
        index,
        fileName: file.name,
        fileSize: file.size,
      });
  ...
  const blobUrlToFile = useCallback(async (blobUrl: string, fileName: string): Promise<File | null> => {
    try {
      ...
      debug('useModelImageManagement', '❌ blobUrlToFile 실패', { blobUrl, fileName, error });
      return null;
    }
  }, []);
  ...
  const getMainThumbnailFile = useCallback(async (): Promise<File | null> => {
    ...
    if (file) {
      debug('useModelImageManagement', '📂 getMainThumbnailFile - 상태/ref에서 가져옴', {
    ...
    if (mainThumbnailUrl && mainThumbnailUrl.startsWith('blob:')) {
      ...
      debug('useModelImageManagement', '📂 getMainThumbnailFile - blob URL에서 생성 시도', {
        mainThumbnailUrl,
        fileName,
      });
    ...
    debug('useModelImageManagement', '📂 getMainThumbnailFile - 파일 없음', {
      mainThumbnailFile: !!mainThumbnailFile,
      refFile: !!mainThumbnailFileRef.current,
      mainThumbnailUrl,
    });
  ...
  const getGalleryImageFiles = useCallback(async (): Promise<File[]> => {
    ...
    if (files.length > 0) {
      debug('useModelImageManagement', '📂 getGalleryImageFiles - 상태/ref에서 가져옴', {
    ...
    if (galleryImageUrls.length > 0) {
      ...
      if (files.length > 0) {
        debug('useModelImageManagement', '📂 getGalleryImageFiles - blob URL에서 생성 시도', {
    ...
    debug('useModelImageManagement', '📂 getGalleryImageFiles - 파일 없음', {
      galleryImageFiles: galleryImageFiles.length,
      refFiles: galleryImageFilesRef.current.length,
      galleryImageUrls: galleryImageUrls.length,
    });
