...
import ModelRegisterPreview from '@/presentation/pages/model/components/ModelRegisterPreview';
import {
  MODEL_PROFILE_CARD_RATIO,
  MODEL_PROFILE_CARD_RATIO_LABEL,
  MODEL_REGISTER_TYPES,
  type ModelRegisterVisibility,
} from '@/presentation/pages/model/config/modelRegisterContent';
import {
...
interface ModelRegisterFormViewProps {
  ...
  onInputChange: (field: keyof ModelFormData, value: string) => void;
...
// test_codex ModelProfileCreatePage 태그 문자열 파싱
...
const getRatioText = (meta?: ImageRatioMeta) => {
  ...
  const ratioStatus = meta.isMatched
    ? '목록 카드에 알맞은 비율입니다.'
    : `${MODEL_PROFILE_CARD_RATIO_LABEL} 카드에 맞춰 중앙 기준으로 잘려 보일 수 있어요.`;

  return `${meta.width}×${meta.height}px · 현재 ${currentRatio.toFixed(2)}:1 · ${ratioStatus}`;
};

// test_codex 기준 모델 등록 폼 본문
const ModelRegisterFormView: React.FC<ModelRegisterFormViewProps> = ({
  formData,
  mainThumbnailUrl,
  galleryImageUrls,
  portfolioFileName,
  ...
  const [modelType, setModelType] = useState<string>(MODEL_REGISTER_TYPES[0]);
  const [height, setHeight] = useState<number>(0);
  const [location, setLocation] = useState('');
  const [tagsInput, setTagsInput] = useState('');
  const [email, setEmail] = useState('');
  const [visibility, setVisibility] = useState<ModelRegisterVisibility>('public');
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      setProfileRatioMeta({
        ...
        isMatched: Math.abs(currentRatio - MODEL_PROFILE_CARD_RATIO) <= 0.04,
      });
    };
    image.src = objectUrl;
      ...
      <PortfolioRegisterFormBody>
        <PortfolioFormCard>
          <PortfolioSectionTitleRow>
            <div>
              <h2>대표 프로필 사진</h2>
              <p>모델 목록과 상세페이지에서 첫 화면에 보이는 이미지입니다.</p>
            </div>
          ...
          <PortfolioProfileUploader $hasFile={Boolean(mainThumbnailUrl)}>
            ...
            {mainThumbnailUrl ? (
              <PortfolioUploadedProfilePreview>
                <img src={mainThumbnailUrl} alt="선택한 모델 프로필 미리보기" />
                <span>사진 변경</span>
              </PortfolioUploadedProfilePreview>
            ) : (
              <>
                ...
                <p>
                  상반신 또는 전신 사진을 권장합니다. 목록에서는 {MODEL_PROFILE_CARD_RATIO_LABEL} 비율로
                  표시됩니다.
                </p>
              </>
            )}
        ...
        <PortfolioFormCard>
          <PortfolioSectionTitleRow>
            <div>
              <h2>기본 정보</h2>
              <p>브랜드가 모델을 검색하고 비교할 때 사용하는 핵심 정보입니다.</p>
            </div>
            <small>필수 {requiredProgress}/5</small>
          </PortfolioSectionTitleRow>
        ...
        <PortfolioFormCard>
          <PortfolioSectionTitleRow>
            <div>
              <h2>프로필 소개</h2>
              <p>브랜드가 촬영 무드와 적합성을 판단하는 영역입니다.</p>
            </div>
          </PortfolioSectionTitleRow>
          ...
          <PortfolioCreateField>
            ...
            <input
              placeholder="예: 화사하고 고급스러운 이미지로 브랜드 무드를 표현합니다."
              maxLength={80}
          ...
          <PortfolioCreateField>
            <span>상세 소개</span>
            <textarea
              placeholder="촬영 가능한 카테고리, 이미지 무드, 촬영 경험, 강점을 구체적으로 작성해주세요."
              maxLength={500}
          ...
          <PortfolioCreateField>
            <span>검색 태그</span>
            <input
              placeholder="#패션 #뷰티 #피팅 #브랜드촬영 #20대"
              value={tagsInput}
        ...
        <PortfolioOptionalFormCard>
          <PortfolioSectionTitleRow>
            <div>
              <h2>활동 자료</h2>
              <p>포트폴리오와 갤러리를 등록하면 브랜드가 촬영 이미지를 더 쉽게 확인할 수 있습니다.</p>
            </div>
          </PortfolioSectionTitleRow>
          ...
          <PortfolioFileUploader $hasFile={Boolean(attachedPortfolioFile)}>
            ...
            {attachedPortfolioFile ? (
              ...
              <>
                ...
                <p>촬영 포트폴리오, 프로필북, 소개서를 PDF/PPT/이미지로 등록할 수 있습니다.</p>
              </>
            )}
          </PortfolioFileUploader>
          ...
          <PortfolioFileUploader $compact $hasFile={galleryImageUrls.length > 0}>
            ...
            {galleryImageUrls.length > 0 ? (
              ...
              <>
                ...
                <p>화보, 룩북, 제품 촬영 이미지를 추가로 보여줄 수 있습니다.</p>
              </>
            )}
          </PortfolioFileUploader>
        </PortfolioOptionalFormCard>
        ...
        <PortfolioContactPolicyFormCard>
          ...
          <PortfolioCreateField>
            ...
            <select
              value={visibility}
              onChange={(event) => setVisibility(event.target.value as ModelRegisterVisibility)}
            >
...
export default ModelRegisterFormView;
